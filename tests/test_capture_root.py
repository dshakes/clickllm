"""ADR-0014, extended to `clickllm_distill`.

`clickllm_prove` was the one MCP tool where an agent names a filesystem path
and the contents land in its context — until `clickllm_distill` grew a
...
to the capture root (`CLICKLLM_HOME`), not the whole filesystem.
"""
...
from clickllm.mcp import _within_capture_root
    ...
    r = mcp.handle(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "clickllm_distill", "arguments": {"captures": captures}},
        }
    )
    return r.get("result", {})
...
def test_a_path_outside_the_root_is_refused(path, tmp_path, monkeypatch):
    monkeypatch.setenv("CLICKLLM_HOME", str(tmp_path))

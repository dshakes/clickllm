...
async fn stalled_giant_frame() -> impl IntoResponse {
    let giant = axum::body::Bytes::from(vec![b'x'; onpar_gateway::sse::MAX_FRAME_BYTES + 1]);
...
async fn a_stalled_frame_past_the_cap_does_not_break_the_stream() {
    ...
    assert!(
        body.len() > onpar_gateway::sse::MAX_FRAME_BYTES,
...
async fn a_split_serves_local_and_never_mirrors_to_the_cloud() {
    ...
    for i in 0..8 {
        let r = reqwest::Client::new()
            ...
            .await
            .unwrap();
        assert_eq!(r.headers().get("x-onpar-backend").unwrap(), "local");
    }
...
async fn failover_reaches_the_cloud_when_local_is_down() {
    ...
    assert_eq!(r.headers().get("x-onpar-backend").unwrap(), "cloud");

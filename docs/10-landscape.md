...
### 3. llama.cpp / MLX / vllm-mlx / MLC-LLM (Apple Silicon)
`llama.cpp` best single-stream + widest models; MLX highest throughput (~230 tok/s aggregate) with spec-decode since mlx-lm 0.21; **vllm-mlx** brings continuous batching to Metal (EuroMLSys '26, ~1,150 tok/s @ 32 concurrency on M4 Max); MLC-LLM best for 64K–128K via paged KV.
**Steal:** all four. This is the entire local story and the reason we can dogfood.
**Gap:** four engines, four config formats, zero guidance on choosing.
...
**Gap:** all start from a human-authored eval set. **Nobody generates the eval set from your production traffic, baselined against your incumbent's actual responses** — the step that actually blocks migrations. (Nearest prior art is llmfit's `quality.rs`, which scores models against 13 generic roles and emits a routing matrix — real, but it answers "best at coding in general", not "matches what GPT-5 did for us". See [ADR-0006](adr/0006-llmfit-source-evaluation.md).) Also: promptfoo being OpenAI-owned is a live strategic risk for a product whose purpose is leaving OpenAI. Plan for Inspect AI as the primary.

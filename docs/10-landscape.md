...
**Gap:** all start from a human-authored eval set. **Nobody generates the eval set from your production traffic** — and that's the step that actually blocks migrations. Also: promptfoo being OpenAI-owned is a live strategic risk for a product whose purpose is leaving OpenAI. Plan for Inspect AI as the primary.

...
> **Amended by [ADR-0004](0004-zero-config-deployment.md):** this is a decision about *strategy* (we do not build an orchestrator), not about *user experience*. Deployment is zero-config — onpar owns every tuning knob and the user is never asked for one.

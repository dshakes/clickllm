...
onpar has five user-facing surfaces: the CLI (24 commands), the MCP server
...
memory-bandwidth roofline. `onpar fit --explain` says so — *"roofline
estimate, not measured"* — and `mcp`, `sdk` and `ui` each carry an
`estimate_basis` beside the number. `onpar fit --json` carried neither: a
...
1. `src/onpar/engine.py` holds the product operations — `fit`, `where`,
...
2. A surface may **format, transport and authenticate**. It may not decide,
   ...
   (ADR-0011).
3. Engine results carry a versioned contract, `onpar.engine/v1`, following
   the existing `onpar.receipt/vN` convention — so a breaking change to a
...
three published contracts at once — the MCP tool schema, `onpar fit --json`,
...
not a declared contract. Meanwhile `onpar fit --json` is documented in the
...
`onpar fit --json` immediately and additively, because a missing disclosure
...
`onpar fit` must keep working under `uvx` with zero runtime dependencies,

//! `clickllm-gateway` — the binary that runs the datapath.
//!
...
use clickllm_gateway::{AppState, Backend, Phase, Route, Router, app};
...
const USAGE: &str = "\
clickllm-gateway — record traffic on its way to the model you already use

USAGE:
    clickllm-gateway --upstream <URL> [OPTIONS]
...
OPTIONS:
    ...
    --capture <PATH>        Capture log (default ./clickllm-captures.log)
...
fn parse_from(args: impl Iterator<Item = String>) -> Result<Args, String> {
    let mut a = Args {
        ...
        capture: Some(PathBuf::from("clickllm-captures.log")),
        key: None,
        percent: 0,
    };
    let mut it = args;

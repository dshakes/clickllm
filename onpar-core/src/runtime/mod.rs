...
//! invocation, a real `InferencePool` — that runs with onpar uninstalled
...
/// An inference engine onpar can plan for, configure, and launch.
...
/// stays runnable if onpar is uninstalled.
pub(crate) fn provenance(runtime: &str, plan: &RuntimePlan, comment: &str) -> String {
    let mut s = format!(
        "{comment} Generated by onpar {} — runtime: {runtime}, model: {}, quant: {}\n\
         {comment} This file is a receipt, not a form. It runs without onpar installed.\n",
        crate::VERSION,
        plan.model.id,
        plan.quant,
    );
...
pub(crate) fn systemd_unit(name: &str, argv: &[String], plan: &RuntimePlan) -> String {
    ...
    let exec = std::iter::once("/usr/bin/env".to_string())
        ...
        .join(" ");
    format!(
        "# onpar — {}, {} @ {} context. Runs with onpar uninstalled.\n\
         [Unit]\nDescription=onpar inference — {}\nAfter=network-online.target\n\

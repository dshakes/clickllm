...
//! Deployment. Output runs with onpar uninstalled (NFR-4).
...
pub(crate) mod tests {
    ...
    fn local_script_is_a_runnable_native_invocation() {
        ...
        // NFR-4: no onpar invocation anywhere in the output.
        assert!(
            !c.lines().any(|l| l.trim_start().starts_with("onpar")),
            "generated config must not shell back into onpar"
        );
    }
    ...
    fn the_systemd_unit_survives_a_slow_start_and_restarts() {
        ...
        assert!(u.contains("onpar"), "provenance header missing");

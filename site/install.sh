...
# `onpar` command. The distribution, the command and the import are all `onpar`.
#
# No npm branch either, and that is a decision rather than an omission. `npx
# onpar-cli` is a shim that execs uvx, then `uv tool run`, then `pipx run` —
# every channel this script already tries first. A branch for it could only
# ever be reached on a machine where all three are missing, which is exactly
# where the shim itself gives up. It would add node as a dependency and
# install nothing.
set -eu

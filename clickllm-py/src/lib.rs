...
//!
//! clickllm is split on a deliberate line: **Rust holds the datapath, Python
...
//!
//! - **Sizing and fit.** Pure Python, zero dependencies, so `uvx clickllm fit`
...
use clickllm_gateway::capture::{Kind, redact as redact_text};
...
/// see `clickllm_gateway::capture::store` for what this protects against and,
...
/// The `clickllm_core` extension module.
#[pymodule]
fn _clickllm_core(m: &Bound<'_, PyModule>) -> PyResult<()> {

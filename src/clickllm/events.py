...
logging, tracing or otel across `src/clickllm/` returned no hits at all. So a
...
**Zero runtime dependencies.** `clickllm fit` must work under `uvx` with nothing
...
they turn off. Nothing is emitted until `CLICKLLM_LOG` is set.
"""
...
#: Where events go, and the only thing that turns them on. `CLICKLLM_LOG=debug`
#: for stderr, or `CLICKLLM_LOG=/path/to/file.log` for a file. There is
...
def configure(spec: str | None = None) -> bool:
    """Turn events on from `CLICKLLM_LOG`, and report whether they are on.

...
by task shape, and writes the eval set `clickllm prove` consumes.
...
def state_dir() -> Path:
    ...
    `CLICKLLM_HOME` first, so a user can put a log full of production prompts on
    a volume they chose deliberately rather than in their home directory.
    """
    return Path(os.environ.get("CLICKLLM_HOME") or Path.home() / ".clickllm")
...
def find_gateway() -> Path | None:
    ...
    """
    override = os.environ.get("CLICKLLM_GATEWAY_BIN")
    if override:
        p = Path(override)
...
def distill(
    ...
    *,
    ...
    Returns the eval-set document `clickllm prove` reads, and a report of what
...
def demo() -> None:
    ...
    usage = (
        Path(__file__).resolve().parents[2] / "clickllm-gateway" / "src" / "main.rs"
    ).read_text()

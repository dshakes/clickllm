...
digest with `clickllm receipt` and no trust in this page.
...
def render(receipt: Receipt, *, title: str = "Migration briefing") -> str:
    ...
    # `clickllm receipt` and get the same digest, or find that they cannot.
    parts.append(
        ...
        "<code>receipt.json</code> and run <code>clickllm receipt receipt.json</code>; "

...
def sources() -> list[Path]:
    ...
    - ``$ONPAR_CATALOG`` — ``os.pathsep``-separated files or directories,
      for a one-off or a CI pin.
    - ``~/.config/onpar/models.d/*.json`` — a drop-in directory, read in
      sorted order. This is the durable one: adding a fleet is `cp` and nothing
      else, and removing it is `rm`.
    """
    found = [CATALOG_PATH]
    for entry in os.environ.get("ONPAR_CATALOG", "").split(os.pathsep):
        if not entry.strip():
            continue
        p = Path(entry).expanduser()
        found.extend(sorted(p.glob("*.json")) if p.is_dir() else [p])
    drop_in = (
        Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "onpar" / "models.d"
    )
    if drop_in.is_dir():
...
def load() -> tuple[ModelSpec, ...]:
    ...
    Re-reads whenever a catalogue file changes, so `onpar catalog-add` takes

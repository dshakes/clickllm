...
class Fit:
    ...
    def aggregate_tokens_per_sec(self) -> float | None:
        ...
        concurrency 32. `onpar host` then printed that beside hosted providers'
...
class Placement:
    ...
    def moe_batch_optimism(self) -> float | None:
        ...
        throughput it divides — are what `onpar where`, `onpar host` and
    ...
    def cost_per_mtok_usd(self) -> float | None:
        ...
        overstated self-hosting by roughly the batch factor — while `onpar
...
def recommend_runtime(hw: Hardware, context: int, concurrency: int) -> tuple[str, str]:
    """(engine, why) — the engine `onpar run` would actually start here.

    This used to carry its own table, and the two disagreed. On Apple silicon at
    concurrency 8, `onpar fit` printed `runtime -> vllm-mlx` while `onpar
    ...
    `launch._launchable_alternative` — the exact function `onpar run`'s own
    ...
    # concurrency it picks llama.cpp — correct, and `onpar run` then refuses
    ...
    if alt is None:
        return (name, f"{why} NOTE: onpar cannot launch {name} — no verified flag dialect.")
    ...
    return (
        ...
        f"onpar has no verified flag dialect for it, so `onpar run` would refuse "
        f"here — nothing launches at concurrency {concurrency} on this hardware. "
        f"`onpar run --concurrency {alt_concurrency}` is what starts {alt_name}; "

...
`onpar fit` sizes against the machine it is running on. In a cluster that is
...
def read_cluster(context: str | None = None, timeout: int = 20) -> list[Node]:
    ...
    would need re-implementing — and it keeps `onpar fit` free of runtime
    dependencies.
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.k8s.nodes`."""
    payload = {
        "items": [
            {
                "metadata": {

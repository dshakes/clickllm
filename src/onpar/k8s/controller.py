...
Everything that decides anything lives in [`onpar.k8s.reconcile`], as a pure
...
that, is already installed wherever this is useful, and keeps `onpar fit` free
...
from onpar.k8s.nodes import read_cluster
from onpar.k8s.reconcile import Reconciled, reconcile
...
class Kubectl:
    ...
    def workloads(self, namespace: str | None = None) -> list[dict]:
        ...
        out = self.run(["get", "inferenceworkloads.onpar.dev", *scope, "-o", "json"])
...
REGRESSION_ANNOTATION = "onpar.dev/regressed"
REGRESSION_REASON_ANNOTATION = "onpar.dev/regression-reason"
    ...
    while max_passes is None or passes < max_passes:
        try:
            for ref, r in reconcile_once(kc, namespace, nodes, regression_check):
                ...
                print(f"[onpar] {ref}: {state}{extra}", flush=True)
            ...
            print(f"[onpar] pass failed: {e}", flush=True)
        passes += 1
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.k8s.controller`."""
    from onpar.k8s.nodes import GPU_MEMORY_LABEL
    ...
    from onpar.k8s.nodes import from_json
...
def main(argv: list[str] | None = None) -> int:
    """Entrypoint. `python -m onpar.k8s.controller` runs the loop.
    ...
    p = argparse.ArgumentParser(prog="onpar-controller")

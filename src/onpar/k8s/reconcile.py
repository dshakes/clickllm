...
glue around it (see [`onpar.k8s.controller`]).
...
from onpar.k8s.nodes import Node
from onpar.plan import Engine, Plan, Requirements, Workload, plan
...
#: onpar never wraps an engine (NFR-4), so these are upstream, not ours.
IMAGES = {
...
def deployment_for(
    ...
    own* flags — a config that runs with onpar uninstalled (NFR-4). Nothing
    ...
    labels = {"app.kubernetes.io/name": name, "app.kubernetes.io/managed-by": "onpar"}
    ...
    return {
        ...
        "metadata": {
            ...
            "annotations": {
                ...
                # what was chosen and that it runs without onpar installed.
                "onpar.dev/engine": p.engine.value,
                "onpar.dev/engine-reason": p.engine_why,
                "onpar.dev/standalone": "these args are the engine's own; "
                "this Deployment runs with onpar uninstalled",
                ...
                # on its own.
                **({"onpar.dev/not-expressed": "; ".join(gaps)} if gaps else {}),
            },
        },
        "spec": {
            "replicas": replicas,
...
def reconcile(
    ...
    *,
    ...
    Args:
        ...
        nodes: cluster nodes, from [`onpar.k8s.nodes`].
            ...
            status={
                "conditions": [
                    condition(
                        ...
                        f"against {node.name}. Add it with `onpar catalog-add "
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.k8s.reconcile`."""
    from onpar.k8s.nodes import GPU_MEMORY_LABEL, GPU_PRODUCT_LABEL, from_json

...
`onpar fit` is pure Python with zero dependencies, and that is a promise worth
keeping: `uvx onpar fit` has to work on a laptop with nothing installed. A
...
`onpar-py/src/lib.rs`. The short version: only the capture format and the
...
INSTALL_HINT = "pip install onpar-core"
...
def _skew() -> str | None:
    ...
    if got in COMPATIBLE:
        return None
    return (
        f"_onpar_core {got} does not match this package "
...
def why_unavailable() -> str | None:
    ...
    if _ext is None:
        return (
            ...
            f"Reading captured traffic needs it; `onpar fit` does not."
        )
    return _skew()

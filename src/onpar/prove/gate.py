...
1. **An LLM judge alone never moves traffic.** A cluster whose entire case rests
   ...
   *provided the judge has been calibrated*. An instrument nobody checked against
   ...
   pass. See [`Calibration`][onpar.prove.judge.Calibration].
...
from onpar.prove.equivalence import DEFAULT_EQUIVALENCE_BAR, ClusterScore, check_bar
from onpar.prove.graders import ItemResult, Tier
from onpar.prove.judge import Agreement, Calibration
from onpar.prove.stats import MIN_SAMPLES_FOR_CONFIDENCE, Interval
...
def read_cluster(cluster: str, name: str, share: float, results: list[ItemResult]) -> Reading:
    ...
    Wraps [`score_cluster`][onpar.prove.equivalence.score_cluster] rather than
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.prove.gate`."""
    from onpar.prove.equivalence import ClusterScore
    ...
    from onpar.prove.judge import Calibration

...
:mod:`onpar.distill`, so the thing being measured is the thing you actually
run.
...
:mod:`onpar.prove.gate` and terminates at a human. An agent is welcome to
...
step: :mod:`onpar.prove.collect`, or ``onpar prove --candidate-endpoint``.
...
def shares_from_clusters(clusters: list) -> tuple[dict[str, float], dict[str, str]]:
    """Traffic shares and display names from :mod:`onpar.distill` clusters.
    ...
    *,
    ...
    Args:
        items: the eval set, each carrying both answers. From
            :mod:`onpar.distill` in the product flow.

...
from onpar.prove.cost import Saving
from onpar.prove.cost import saving as cost_saving
...
from onpar.prove.stats import samples_needed, wilson
...
SUPPORTED_FORMATS = ("onpar.receipt/v1", FORMAT)

#: Exactly the fields `onpar.receipt/v1` carried. Frozen — this is a
...
class Receipt:
    ...
    def __post_init__(self) -> None:
        ...
        `onpar receipt`, `onpar guard` and the box, and a file with
        ...
        if self.format == "onpar.receipt/v1" and (self.incumbent_cost or self.candidate_cost):
            raise ValueError(
                "a onpar.receipt/v1 document cannot carry a cost — its digest "
    ...
    def digest(self) -> str:
        ...
        """
        payload = asdict(self)
        if self.format == "onpar.receipt/v1":
    ...
    def from_json(cls, text: str) -> Receipt:
        ...
        # This is the disk-ingest path behind `onpar receipt`, `guard` and
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.prove.receipt`."""

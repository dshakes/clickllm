"""onpar CLI. Stdlib only — `uv run onpar fit` must work with zero install."""
...
if TYPE_CHECKING:  # imported lazily at the call sites — `onpar fit` must stay cheap
...
def cmd_fit(args: argparse.Namespace) -> int:
    ...
    print(
        "\n  ~tok/s is a memory-bandwidth roofline, not a measurement — `onpar measure` measures."
    )
    print("  onpar fit --explain <model-id>   # show the arithmetic\n")
    return 0
...
def cmd_where(args: argparse.Namespace) -> int:
    ...
    if spread > 1.0:
        notes.append(
            ...
            f" {spread:.1f}x wide — see `onpar fit --explain`."
        )
    print("\n".join(notes) + "\n")
    return 0
...
def cmd_catalog_add(args: argparse.Namespace) -> int:
    ...
    — the same ground truth `onpar catalog` verifies against — and written to
    ...
    print(f"  onpar fit --explain {model_id}\n")
    return 0
...
def _drop_in_dir() -> pathlib.Path:
    ...
    return pathlib.Path(base) / "onpar" / "models.d"
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    traffic onto it is a separate, human decision — see `onpar prove`.
    """
    from . import launch
...
def _serve_until_signalled(ep) -> int:
    ...
    into `ep.wait()`, so a signal killed onpar and left the engine serving,
    holding the GPU, with the port still open. Measured against a real run:
    onpar gone, `mlx_lm` alive, endpoint still answering.
    ...
    process reaches onpar alone: `kill`, systemd, `docker stop`, a Kubernetes
...
def cmd_host(args: argparse.Namespace) -> int:
    ...
    if not args.write:
        print(f"  onpar host {m.id} --write ./deploy   # emit the deploy files")
        print(f"  onpar host {m.id} --free-only        # only what costs nothing")
    ...
    print(
        "\n  Run these yourself. onpar never authenticates to a provider, and\n"
...
def cmd_cache(args: argparse.Namespace) -> int:
    ...
    if u.entries:
        print("  onpar cache pin <repo>              # never evict this one")
        print("  onpar cache evict --budget 200G     # show what would go\n")
    return 0
...
def cmd_box(args: argparse.Namespace) -> int:
    ...
    The push commands are printed for you to run. onpar holds no registry
    ...
    if not args.write:
        print(f"  onpar box {b.model.id} --write ./{b.model.id}-box   # emit the artifact")
        print(f"  onpar box {b.model.id} --receipt receipt.json       # attach the proof")
    ...
    for command in b.push_commands(args.write):
        print(f"    {command}")
    print(
        "\n  Run these yourself. onpar holds no registry credential, reads no\n"
...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    if args.list:
        ...
        print("\n  onpar catalog-add <repo> --params-b <N> --network   # promote\n")
        return 0
...
def cmd_observe(args: argparse.Namespace) -> int:
    ...
    if binary is None:
        print(
            ...
            "  From source:   cargo build --release -p onpar-gateway\n"
            "  Or point at one: ONPAR_GATEWAY_BIN=/path/to/onpar-gateway\n\n"
            "  `onpar fit` needs none of this — the gateway is only for capture.\n"
        )
        return 1
    ...
    # keeps its value the moment onpar-core is installed. Refusing would
    ...
    if not args.no_capture and not core.available():
        print(
            ...
            f"  `onpar distill` needs it. Install it before you rely on this run.\n",
            file=sys.stderr,
        )
...
def cmd_distill(args: argparse.Namespace) -> int:
    """Turn a capture log into the eval set `onpar prove` reads."""
    from . import core, observe
    ...
    if not log.exists():
        print(f"\n  No capture log at {log}. Run `onpar observe` first.\n")
        return 1
...
def cmd_migrate(args: argparse.Namespace) -> int:
    ...
    if not log.exists() or not key.exists():
        print(f"\n  No captures at {log}. Run `onpar observe` first.\n")
        return 1
...
#: Mirrored from `onpar.measure` so building the parser does not import it.
...
def cmd_measure(args: argparse.Namespace) -> int:
    ...
    conditions that produced it. See issue #80 and `onpar.measure`.
    """
    ...
    result = M.measure(
        ...
        api_key=os.environ.get("ONPAR_API_KEY", ""),
    )
...
def cmd_catalog(args: argparse.Namespace) -> int:
    ...
    if args.apply:
        ...
        if any(p.significant for p in report.changed):
            print("  Memory figures changed — re-run `onpar fit` before deploying.")
    print()
    return 0
...
def cmd_discover(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        "\n  run `onpar where <model>` and prove it on your traffic first.\n"
    )
    return 0
...
def cmd_guard(args: argparse.Namespace) -> int:
    ...
    Heavy imports stay inside the handler so `onpar fit` remains stdlib-only.
    """
    from datetime import date
...
def _refuse_ungraded(items: list, *, endpoint: str | None, source: str) -> None:
    ...
    `onpar distill` writes precisely this file, deliberately: the candidate
    ...
    place, because it was written for `onpar prove` and `onpar migrate`
...
def cmd_prove(args: argparse.Namespace) -> int:
    ...
    The kiosk over `onpar.prove`. Deliberately has no flag that moves traffic:
    ...
    #
    # `onpar distill` writes exactly this file, deliberately: the candidate
    ...
    result = suite(
        ...
        # What the receipt was issued against, so `onpar guard` can later
...
def cmd_desktop(args: argparse.Namespace) -> int:
    ...
    print("  It starts `onpar ui` and opens it. Nothing is bundled and")
...
def cmd_version(args: argparse.Namespace) -> int:
    ...
    print("  distribution  onpar")
    print(f"  python        {sys.version.split()[0]}")
    return 0


def cmd_upgrade(args: argparse.Namespace) -> int:
    """`onpar upgrade`. Prints how to upgrade; does not do it.

    Deliberately not self-updating. onpar may be installed by uv, pipx or
    ...
    print("    pipx upgrade onpar")
    print("    python3 -m pip install --upgrade onpar")
    print()
...
def _converse(parser: argparse.ArgumentParser) -> int:
    """A bare `onpar`: one question at a time, until there is an answer.
    ...
    **Only when stdin is a terminal.** A bare `onpar` in a script, a
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Reads ONPAR_LOG and does nothing if it is unset, which is the default.
    ...
    # Both spellings. `--version` is the convention; `onpar version` is what
    ...
    p.add_argument(
        ...
        "-V",
        action="version",
        version=f"onpar {__version__}",
    )
    # Not required: a bare `onpar` opens a conversation instead of printing
    ...
    ob.add_argument("--capture", help="where the log goes (default: ~/.onpar/captures.log)")
    ...
    di.add_argument("--capture", help="the log to read (default: ~/.onpar/captures.log)")
    di.add_argument("--key", help="the capture key (default: ~/.onpar/capture.key)")
    ...
    mg.add_argument("--capture", help="capture log (default: ~/.onpar/captures.log)")
    mg.add_argument("--key", help="capture key (default: ~/.onpar/capture.key)")
    mg.add_argument("--state", help="migration state file (default: ~/.onpar/migration.json)")
    ...
    pv.add_argument(
        ...
        "e.g. http://127.0.0.1:8000/v1 (from `onpar run`)",
    )
    pv.add_argument(
        "--candidate-model",
    ...
    pv.add_argument(
        ...
        type=float,
        help=(
            "what the candidate would cost per month. `onpar fit` and "
            "`onpar host` both estimate this for self-hosting; pass the "
            "figure you believe."
        ),
    )
    pv.add_argument(
    ...
    pv.add_argument(
        ...
        help=(
            ...
            "`onpar guard` can later detect a silent provider-side swap. Without "
    ...
    # unsupported platform, and it was not in this tuple — so `onpar desktop`

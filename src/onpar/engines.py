...
SOURCES = {
    ...
    # GPU box, `ONPAR_REQUIRE_ENGINES=1 pytest tests/test_engine_flags.py
...
class LlamaCppAdapter(Adapter):
    ...
    it is the most predictable Metal option — and `onpar run` then refused for
    want of a dialect. `onpar fit` had to apologise for its own advice.
...
class LlmdAdapter(VllmAdapter):
    ...
    flag vocabulary is exactly how `onpar fit` came to recommend an engine
    ...
    Deployments. Those belong to `onpar.k8s.reconcile`, not to an argv
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.engines`."""

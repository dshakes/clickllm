"""`onpar host` — where to run a model this machine cannot.

[`launch`][onpar.launch] refuses when the model does not fit the box in front
...
writes files you run yourself. The provider bills you directly and onpar
...
#: over the day `onpar where` should rank it too.
...
REGISTRY: tuple[Provider, ...] = (
    ...
    Provider(
        ...
        steps=(
            ...
            "deploy. The endpoint URL and its token are yours — onpar never sees them",
        ),
        notes=(
...
class HostOption:
    ...
    provider: Provider
    offer: Offer
    #: The sizing answer, from the same solver `onpar fit` uses. Carries the
...
def survey(
    ...
    *,
    ...
    Sizing is [`fit`][onpar.fit]'s, not a second implementation: each offer's
    hardware profile becomes a `Hardware` and goes through the same solver that
    answers `onpar fit`. A hosting recommendation that disagreed with the
...
class Artifact:
    ...
    Standalone by contract (NFR-4): nothing generated here imports onpar,
...
def _header(
    ...
    return [
        ...
        *_field(
            ...
            f"{option.memory_gb:g} GB at this shape. onpar host ranks every "
        ...
        *_field(
            "STANDALONE",
            "this file has no onpar dependency and deploys with it "
            "uninstalled. Everything below is the target platform's own config.",
        ),
        *_field(
            "SECRETS",
            "onpar holds none and asks for none. Anything secret below is "
    ...
    compose = "\n".join(
        [
            ...
            *[f"      - {json.dumps(a)}" for a in args],
            ...
            "    # from a file onpar wrote — this line names it and nothing more.",
            "    environment:",
    ...
    app = "\n".join(
        [
            ...
            *head,
            ...
            "",
            f'app = modal.App("onpar-{option.fit.model.id}", image=image)',
            "",
            "",
            "@app.function(",
            "    gpu=GPU,",
    ...
    app = "\n".join(
        [
            ...
            *head,
            ...
            f"# onpar sized this Space at {option.fit.quant}, which transformers cannot",
...
def artifact(
    ...
    *,
    ...
    Returns:
        An [`Artifact`]. Nothing in it imports onpar (NFR-4).
    ...
    if not weights:
        raise ValueError(
            ...
            f"`onpar catalog-add <repo> --params-b {model.params_b:g} --network`, "
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.host`. Touches no network."""
    import tempfile
    ...
    # Artifacts: standalone, stamped, and free of anything onpar.
    with tempfile.TemporaryDirectory() as tmp:
        for provider_id in ("runpod", "modal", "hf-zerogpu", "hf-endpoints"):
            ...
            for name, content in art.files:
                assert "onpar host — generated 2026-07-28" in content, (provider_id, name)
                assert "import onpar" not in content, (provider_id, name)
                assert "onpar." not in content.replace("onpar host", ""), (provider_id, name)

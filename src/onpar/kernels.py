"""The kernel seam — onpar does not write kernels, it proves them.
...
eyeball a few outputs, ship — cannot tell those apart. `onpar prove` can,
...
def verification_plan(claim: KernelClaim) -> list[str]:
    ...
    if claim.bit_identical:
        ...
        steps.append(
            ...
            f"so equivalence is a statistical question: run `onpar prove` and "
...
def scaffold(plugin: Plugin, claim: KernelClaim | None = None) -> dict[str, str]:
    ...
    files["pyproject.toml"] = (
        ...
        f"# Group semantics differ; see onpar.kernels.ENTRY_POINT_GROUPS.\n"
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.kernels`."""

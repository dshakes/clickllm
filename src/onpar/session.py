...
def _prove_path(model_id: str) -> list[str]:
    ...
    It was in two: `answer()` printed a `onpar prove` line and the PROVE
    ...
    """
    return [
        "  onpar observe                       record real requests, redacted before storage",
        "  onpar distill                       turn them into an eval set",
        f"  onpar prove evalset.json --candidate {model_id} \\",
        "      --incumbent <your-current-model> --incumbent-cost <$/mo> \\",
        "      --candidate-cost <$/mo> --traffic-window '14 days'",
        "  onpar brief receipt.json --out brief.html",
    ]
...
class Session:
    ...
    def _what_would_fit(self) -> str:
        ...
        out += ["", "Run `onpar where <model>` to see which hardware serves a model you want."]
        return "\n".join(out)

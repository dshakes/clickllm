"""`onpar cache` — what proving N models left on your disk, and what to remove.
...
store: a cache onpar owned would be a cache nothing writes to, which is the
...
[`apply_eviction`] refuses to act unless it is told twice, and `onpar cache
    ...
    def render(self) -> str:
        ...
        if self.over_by:
            lines.append(
                ...
                f"onpar cache evict --budget {human(self.budget_bytes or 0).replace(' ', '')}"
            )
        lines.append("")
        return "\n".join(lines)
...
def apply_eviction(plan: Plan, *, confirm: bool = False, root: Path | None = None) -> Removal:
    ...
    and `onpar cache evict` only says it when the user passed `--yes`.
    """
    if not confirm:
        return Removal()
    root = root or hub_dir()
...
# --------------------------------------------------------------------------- #
# Pins and budget — onpar's state, never HF's cache
# --------------------------------------------------------------------------- #


@dataclass(frozen=True, slots=True)
class State:
    """Pins and the configured budget. Lives in onpar's state dir.

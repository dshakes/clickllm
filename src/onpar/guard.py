...
def check(
    ...
    *,
    ...
    #
    # Reachable by default rather than in some corner: `onpar prove` had no
    ...
    if fingerprints and not receipt.fingerprints:
        found.append(
            Finding(
                ...
                f"`onpar prove --fingerprints` to make the model-change check "
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.guard`."""
    from onpar.prove.equivalence import CandidateReport, ClusterScore
    from onpar.prove.receipt import issue

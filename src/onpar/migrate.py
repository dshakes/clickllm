    ...
    *,
    ...
    if result.receipt.judge_model:
        raise ValueError(
            ...
            "and use `onpar prove` directly when you want a judge."
        )
...
def install_schedule(*, interval_hours: int = 24, state: str = "") -> tuple[str, str]:
    ...
    fragment = f"{every} onpar migrate --step{arg} >> ~/.onpar/migrate.log 2>&1\n"
...
def demo() -> None:
    ...
    assert "onpar migrate --step" in fragment and "*/6" in fragment
    print("migrate: ok")

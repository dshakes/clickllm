"""`onpar run` — from a catalogue id to a served endpoint you can curl.
...
repos on MLX (see [`MlxAdapter`][onpar.engines.MlxAdapter]), which is why
...
def _cache_write(path: Path, key: str, repo: str) -> None:
    ...
    Two `onpar run` invocations resolving different models at once used to
    ...
    *,
    ...
    if engine == "ollama":
        ...
        if not tag:
            return Refusal(
                ...
                next_step=(
                    f"onpar run {model.id} --engine ollama --tag <ollama-tag>, e.g. "
    ...
    if not tried:
        return Refusal(
            ...
            next_step=f"onpar catalog-add <repo> --params-b {model.params_b:g} --network",
        )
    ...
    return Refusal(
        ...
        next_step=(
            ...
            "`onpar catalog-add <repo> --network`"
        ),
    )
...
def _does_not_fit(model: ModelSpec, hw: Hardware, context: int, concurrency: int) -> Refusal:
    ...
    return Refusal(
        ...
        next_step=(
            f"`onpar host {model.id}` ranks the places that can run it, free "
            f"tiers first, and writes a deploy artifact. `onpar where "
    ...
    if adapter is None:
        ...
        return Refusal(
            ...
            next_step=(
                ...
                if alt
                else f"`onpar build` prints the reasoning for a {engine_name} deployment "
                f"you run yourself"
            ),
        )
...
def _healthy(base: str, model: str = "onpar-probe", timeout: float = 10.0) -> bool:
    ...
    and the cost of believing so is the worst kind of wrong answer: `onpar
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.launch`. Touches no network."""
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert isinstance(no, Refusal) and "onpar host" in no.next_step, no

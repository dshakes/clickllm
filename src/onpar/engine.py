...
That is not a tidiness problem. `onpar fit --json` was the one that dropped
...
#: (`onpar.receipt/v2`, `onpar.box/v1`). A surface may render these
...
def fit(
    ...
    The single computation behind `onpar fit`, `onpar_fit`, `sdk.fit` and
...
def demo() -> None:
    ...
    assert CONTRACT.startswith("onpar.engine/"), CONTRACT
    print("engine: ok")

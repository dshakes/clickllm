"""`onpar box` — the artifact ADR-0005 committed to, and nothing more.
...
environment — the same boundary [`host`][onpar.host] keeps. A box is a file
...
ARTIFACT_TYPE = "application/vnd.onpar.box.v1+json"

FORMAT = "onpar.box/v1"
LOCK_FORMAT = "onpar.weights-lock/v1"
BENCH_FORMAT = "onpar.bench/v1"
...
ABSENT = (
    {
        "path": "targets/cpu/",
        "reason": (
            "the CPU fallback is llama.cpp and onpar has no verified flag "
...
class Box:
    ...
    def push_commands(self, directory: str | Path = ".") -> tuple[str, ...]:
        """The exact commands to publish this box. You run them, not onpar.
...
def _yaml(value: Any, indent: int = 0) -> list[str]:
    ...
    Written rather than imported because `onpar fit` and everything it
...
def _header(model: ModelSpec, generated: str, extra: list[str]) -> list[str]:
    ...
    return [
        f"onpar box — generated {generated}",
        "",
        ...
        *_field(
            "STANDALONE",
            "this file has no onpar dependency and runs with it uninstalled. "
...
def _kubernetes(head: list[str], t: TargetPlan, p: Plan, image: str, port: int) -> str:
    ...
    and a cluster that onpar reconciles from disagreeing about the image, the
...
def _native(head: list[str], t: TargetPlan, argv: list[str], install: str) -> list[tuple[str, str]]:
    ...
    spec = {
        ...
        "standalone": (
            f"the engine's own flags. This runs with onpar uninstalled; "
    ...
    if receipt is None:
        return {
            ...
            "note": (
                ...
                "`onpar prove` measures a candidate against captured traffic "
...
def _readme(box_model: ModelSpec, generated: str, targets, skipped, weights, ref, proof) -> str:
    lines = [
        f"# {box_model.name} — onpar box",
        "",
        f"Generated {generated} by `onpar box {box_model.id}`. "
    ...
    lines += [
        ...
        "## Publishing",
        "",
        "onpar does not push. It holds no registry credential, reads no token, "
    ...
    *,
    ...
    Args:
        ...
        exists: repo existence check, forwarded to
            [`resolve_weights`][onpar.launch.resolve_weights].
...
def demo() -> None:
    """Self-check. Run with `python -m onpar.box`. Touches no network."""
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Nothing generated needs onpar installed.
        for name, content in box.files:
            assert "import onpar" not in content, name
            assert "pip install onpar" not in content, name
